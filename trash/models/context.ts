...
class ParserScope {
    ...
    constructor(analyzers: ParserScope['analyzers'], filePath: string, content: TokenElement[] = []) {
    ...
    public invalidReferenceCall(reference: string, position: Position) {
        this.messages.push({
            type: 'error', message: `The reference "${reference}" was not found.`, position
        })
    }

    public unexpectedTokenDeclaration(data: string, position: Position) {
        this.messages.push({
            type: 'error', message: `An unexpected token declaration: "${data}".`, position
        })
    }

    // - - - 
    public assignLastElement(nodes: NodeElement[], node: NodeElement, indent: number = this.indent, dist = 0): void {
        ...
        if (entry.childNode !== null) {
            return this.assignLastElement([entry.childNode], node, indent, dist + 1)
        }
        ...
        return this.assignLastElement(entry.childNodes, node, indent - 1, dist + 1)
    }
